...
type SlashCommand = "new" | "resume" | "rename" | "cache" | "clear-cache" | "quit";
...
const contextUsageWithDraft = (
  ...
  const estimatedTotal = usage.estimated_thread_tokens + draftTokens;
...
const App = ({args}: {args: Args}) => {
  ...
  const loadThreadSession = useCallback(
    ...
    [args.coreUrl, refreshContextUsage],
  );
...
const ContextUsageLine = ({usage}: {usage: ContextUsageResponse | null}) => {
  ...
  return (
    <Text color={color}>
      ...
      {usage.estimated_draft_tokens > 0
        ? ` (${formatTokenCount(usage.estimated_thread_tokens)} history + ${formatTokenCount(usage.estimated_draft_tokens)} draft)`
        : ""}{" "}
...
if (args.help) {
  process.stdout.write(
    "Usage: academic-agent [resume [NAME]|--resume [NAME]] [--core-url URL] [--idea TEXT] [--once]\n" +
      "Commands: /new, /new IDEA, /resume, /resume NAME, /rename, /rename NAME, /cache, /clear-cache, /quit\n" +
